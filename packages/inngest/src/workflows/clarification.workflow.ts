    ...
    if (!hasEnoughInfo) {
      const decision = await step.run("ask-claude-for-next-question", async () => {
        return chatCompleteJSON<ClarificationDecision>([
          {
            role: "system",
            content: `You are Alfred, a product assistant clarifying a feature request before a PRD is written. Make sure these points are covered before you stop asking questions:\n- ${CLARIFICATION_CHECKLIST}\n\nPrefer multiple-choice questions over open-ended ones whenever the answer fits a short list of concrete choices (e.g. target audience, scope, priority level). Give 2-5 short, mutually distinct options in "options". Only leave "options" empty when the answer genuinely needs free text (e.g. asking for specific numbers, names, or open descriptions).\n\nRespond with ONLY JSON: { "done": boolean, "question": string (only if done is false), "options": string[] (only if done is false and a multiple-choice question makes sense) }. Set "done": true once you have enough information to write a thorough PRD, or once you've already asked ${MAX_ALFRED_QUESTIONS} questions.`,
          },
          ...messages.map((m) => ({
